...
const KO_STOPWORDS = new Set([
  ...
  // 식품 카테고리 우산어 — 특정 제품이 아니라 넓은 범주라 발굴 랭킹에 뜨면 노이즈.
  // (단독 토큰만 매칭 → "소금빵"·"마늘빵"·"카페라떼" 같은 복합어는 안전하게 통과)
  "빵", "베이커리", "카페", "맛집", "먹스타그램", "존맛", "꿀맛", "유행", "신메뉴",
  "가성비", "핫플", "맛스타", "디저트카페", "빵집", "베이글집",
  // 수량·구성 표기
  "종류별", "세트", "묶음", "낱개", "대용량",
...
export const KO_VERB_ENDING =
  /(?:세요|해요|네요|어요|아요|에요|예요|봐요|봐|군요|는데|니까|잖아|거든|더라|던데|는다|한다|된다|왔다|갔다|봤다|했다|났다|겠다|드세요|보세요|다면|하지|않은|보자|하자|먹자|없네|있네|네|어본|아본|해본|여본|어봄|아봄|습니다|습니까|습니꽈|입니다|됩니다|랍니다|더라고요|더라고|거예요|거에요)$/;
