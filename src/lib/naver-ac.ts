...
import { KO_VERB_ENDING, isGenericCompound } from "./cooccurrence";
...
const INFO_TAILS = [
  ...
  // 작품·인물 정보 (시드가 총칭어일 때 딸려온다: "김부장 등장인물", "최애의 사원 원작")
  "등장인물", "인물", "원작", "결말", "출연진", "몇부작", "웹툰", "만화", "소설",
  // 생활정보·부업 (제품이 아니라 방법 검색: "복숭아 보관법", "쇼핑쇼츠 부업")
  "보관법", "손질법", "세척법", "부업", "수익", "정산", "하는법", "고르는법",
];
...
export function pickCandidates(seed: string, completions: string[]): AcCandidate[] {
  ...
  completions.forEach((raw, i) => {
    ...
    // 총칭 성분만으로 이뤄진 말은 제품이 아니다 ("편의점 신상", "아이돌 간식", "쇼핑쇼츠").
    // 유튜브 단계에서 새어 나온 총칭어가 시드가 되면 여기서 총칭 완성어로 증폭되므로 같은 게이트를 건다.
    if (isGenericCompound(term)) return;
    seen.add(key);
